...
macOS : macOS 26.0以降 (それ以前のバージョンでは動作しない可能性があります)

Windows : Windows 11以降 (それ以前のバージョンでは動作しない可能性があります)
...
5. ExPing.def 形式のファイルを読み込むことも可能です。その場合は、この画面の時にファイルをドロップしてください。
...
> - ブロックサイズは IP 最大パケットサイズ (65,535) から 標準的なIPv4 ヘッダ (20 bytes) と ICMP ヘッダ (8 bytes) を差し引いた 65,507 bytes が上限です。
  - ただし一般的なLAN/インターネットへのPingは1460バイト程度が最大で、LAN内でジャンボフレームを許可した場合でも8960バイト程度となります。
  - 一般設定 > パケットのフラグメントを禁止する を有効にすると、ブロックサイズは一般的に1460バイト以下に制限されます。
...
ソースコードの閲覧・改変・再配布は自由に行えます。教育目的や商用目的での利用も可能です。ぜひ自分のニーズに合わせてカスタマイズしてみてください。
