            ...
            <div className={styles.settingsGrid} style={{ gridTemplateColumns: '1fr' }}>
                ...
                <div className={styles.fieldRow}>
                    ...
                    <div className={styles.pathInputGroup} style={{ opacity: settings.ng.playSound ? 1 : 0.5 }}>
                        <input type="text" value={settings.ng.soundFile} onChange={e => setSettings({ ...settings, ng: { ...settings.ng, soundFile: e.target.value } })} disabled={!settings.ng.playSound} style={{ flex: 1 }} />
                        <button className={styles.btnSmall} onClick={() => selectFile('sound')} disabled={!settings.ng.playSound}>...</button>
                        <button className={styles.btnSmall} disabled={!settings.ng.playSound} onClick={() => playSound(settings.ng.soundFile)}>♪</button>
                    </div>
                </div>
                ...
                <div className={styles.settingsSection}>
                    ...
                    <div className={styles.nestedFields} style={{ opacity: settings.ng.launchProgram ? 1 : 0.5 }}>
                        <div className={styles.fieldRow}>
                            <label style={{ width: '120px', fontSize: '12px' }}>プログラム:</label>
                            <div className={styles.pathInputGroup}>
                                <input type="text" value={settings.ng.programPath} onChange={e => setSettings({ ...settings, ng: { ...settings.ng, programPath: e.target.value } })} disabled={!settings.ng.launchProgram} style={{ flex: 1 }} />
                                <button className={styles.btnSmall} onClick={() => selectFile('program')} disabled={!settings.ng.launchProgram}>...</button>
                            </div>
                        </div>
                        <div className={styles.fieldRow}>
                            <label style={{ width: '120px', fontSize: '12px' }}>オプション:</label>
                        ...
                        <div className={styles.fieldRow}>
                            <label style={{ width: '120px', fontSize: '12px' }}>作業ディレクトリ:</label>
                            <div className={styles.pathInputGroup}>
                                <input type="text" value={settings.ng.programWorkingDir} onChange={e => setSettings({ ...settings, ng: { ...settings.ng, programWorkingDir: e.target.value } })} disabled={!settings.ng.launchProgram} style={{ flex: 1 }} />
                                <button className={styles.btnSmall} onClick={() => selectDir('ng')} disabled={!settings.ng.launchProgram}>...</button>
                            </div>
                        </div>
                    </div>
                </div>
